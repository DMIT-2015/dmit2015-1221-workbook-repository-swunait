...
import dmit2015.GamingPlatform;
...
//            e.printStackTrace();
    ...
    public List<String> mapToTitle() {
        ...
        List<String> resultList = em.createQuery(
                "SELECT g.title FROM VideoGame g ORDER BY g.title", String.class)
                .getResultList();
        em.close();
        return resultList;
    }
    ...
    public List<Double> mapToDistinctPrices() {
        EntityManager em = _emf.createEntityManager();
        List<Double> resultList = em.createQuery(
                "SELECT g.price FROM VideoGame g GROUP BY g.price ORDER BY g.price DESC", Double.class)
                .getResultList();
        em.close();
        return resultList;
    }
    ...
    public Set<Double> mapToPriceSet() {
        EntityManager em = _emf.createEntityManager();
        Set<Double> resultSet = em.createQuery("SELECT g.price FROM VideoGame g ORDER BY g.price DESC", Double.class)
                .getResultStream()
    ...
    public List<VideoGame> sortByPlatformThenTitleThenPriceDescending() {
        ...
        List<VideoGame> resultList = em.createQuery(
                "SELECT g FROM VideoGame g ORDER BY g.platform, g.title, price DESC ", VideoGame.class)
                .getResultList();
        em.close();
        return resultList;
    }
    ...
    public Optional<VideoGame> findFirstByTitleAndPlatform(String title, GamingPlatform platform) {
        EntityManager em = _emf.createEntityManager();
        List<VideoGame> resultList = em.createQuery(
                "SELECT g FROM VideoGame g WHERE g.title = :titleParam AND g.platform = :platformParam", VideoGame.class)
